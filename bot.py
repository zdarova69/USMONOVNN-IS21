...
class MyBot:
    ...
    def run(self):
        ...
        def welcome(message):
            ...
            self.bot.reply_to(message, f"Привет, {self.user_name}! Я бот кода будущего 🤡. Задай мне вопрос: \n Если хочешь что-то найти при помощи меня,напиши search и предмет поиска \n Пример: /search шрек", reply_markup=self.kb)
        #команда для поиска
